...
export default function MovingCityEditForm({ city }: { city: any }) {
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  const [content, setContent] = useState<string>(city.content || "");
  ...
  const onSubmit = async (data: FormData) => {
    setLoading(true);
    try {
      const res = await fetch(`/api/moving/cities/${city.id}`, {
        method: "PUT",
    ...
    <div className="max-w-5xl mx-auto">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">/movers/{city.slug}</p>
        </div>
      </div>
